...
test.describe('HU42 - Detalles Profesionales', () => {
  test('CP-HU-42-ADD4: Cancelar Modal de Nueva Experiencia sin Guardar Datos', async ({ page }) => {
    ...
    // Test ADD4: Cancelar modal sin guardar
    ...
    await page.getByPlaceholder('Ej. Ayudante de Cátedra').fill('Tutor Freelance');
    ...
    // Verificar que NO contiene la nueva experiencia (no debería estar visible)
    const tutorFreelanceText = page.getByText('Tutor Freelance');
    await expect(tutorFreelanceText).not.toBeVisible();
    
    // - Verificar que estamos de vuelta en detalles profesionales
