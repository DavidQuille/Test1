...
test.describe('Confirmar Tutoría Virtual', () => {
  test('R2: Intento sin enlace obligatorio - muestra error', async ({ page }) => {
    ...
    // Esperar modal - buscar por el texto "Confirmar Tutoría" en heading
    ...
    // 5 y 6. Dejar vacío y hacer clic en Confirmar
    await page.getByRole('button', { name: 'Confirmar' }).click();
    
    // Verificar error
    await expect(page.getByText('El enlace de reunión es obligatorio.')).toBeVisible({ timeout: 5000 });
